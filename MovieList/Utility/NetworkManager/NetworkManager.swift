...
//            return completionOnMain(.failure(NetworkError.unreachable))
//        }
        ...
        task = session.dataTask(with: request, completionHandler: {data, response, error in
            ...
            guard error == nil else {
                if let error = error {
                    ...
                    completionOnMain(.failure(ErrorHandler.handleNetworkError(error).error))
                }
                return
            }
            ...
            guard response.statusCode.isSuccess else {
                return completionOnMain(.failure(ErrorHandler.mapNetworkError(response.statusCode).error))
            }
